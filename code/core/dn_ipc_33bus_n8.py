# -*- coding: utf-8 -*-
"""IP-C 33-bus: 补种子500/1000/1500 → n=8 Wilcoxon"""
...
N_BUS = 33
print(f"Device: {DEVICE}  IP-C-33bus n8: extra seeds {NEW_SEEDS}")

# 已验证的5种子rob结果（nai固定，由固定naive模型+固定rng=77决定）
EXISTING_ROB = {
    42:   {'rob10': 0.654, 'rob30': 0.615},
    123:  {'rob10': 0.655, 'rob30': 0.610},
    456:  {'rob10': 0.655, 'rob30': 0.604},
    789:  {'rob10': 0.643, 'rob30': 0.618},
    2024: {'rob10': 0.660, 'rob30': 0.618},
}
...
def gen_batch_robust(rng, n):
    ...
    for _ in range(n):
        ...
        obs_v=V_library[ti,lf_idx,reported]+rng.normal(0,SIGMA,len(reported))
        x=np.zeros(N_BUS*3,dtype=np.float32)
        x[reported]=obs_v; x[N_BUS+reported]=1.0; x[2*N_BUS+reported]=base_P_norm[reported]*lf_grid[lf_idx]
...
print(f"Naive @10%={nai10:.3f}  @30%={nai30:.3f}  (expected ~0.401 / ~0.101)")
...
for seed in NEW_SEEDS:
    ...
    new_results[seed] = {'rob10': rows[0.10]['rob'], 'rob30': rows[0.30]['rob']}
    print(f"Seed {seed}: rob10={rows[0.10]['rob']:.3f}  rob30={rows[0.30]['rob']:.3f}", flush=True)

# 汇总 n=8
all_seeds = [42, 123, 456, 789, 2024] + NEW_SEEDS
all_rob10 = [EXISTING_ROB[s]['rob10'] for s in [42,123,456,789,2024]] + [new_results[s]['rob10'] for s in NEW_SEEDS]
all_rob30 = [EXISTING_ROB[s]['rob30'] for s in [42,123,456,789,2024]] + [new_results[s]['rob30'] for s in NEW_SEEDS]
all_d10 = [r-nai10 for r in all_rob10]
all_d30 = [r-nai30 for r in all_rob30]

print(f"\n{'='*60}")
print(f"IP-C-33bus n=8 SUMMARY  nai10={nai10:.3f}  nai30={nai30:.3f}")
...
print(f"  All d10>0: {all(d>0 for d in all_d10)}  All d30>0: {all(d>0 for d in all_d30)}")
...
with open(f"{SAVE_DIR}\\ipc_33bus_n8_result.txt", 'w', encoding='utf-8') as f:
    f.write('\n'.join(out) + '\n')
print(f"Saved: ipc_33bus_n8_result.txt")
