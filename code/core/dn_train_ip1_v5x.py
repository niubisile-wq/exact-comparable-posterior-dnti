# -*- coding: utf-8 -*-
"""v5X: 缩小负荷扰动 lf~U(0.9,1.1)，其余与v5相同"""
...
LR        = 3e-4
SIGMA     = 0.009
LF_MIN, LF_MAX = 0.9, 1.1   # ← 改动：±10%
N_LF      = 101
LOG_EVERY = 10000
np.random.seed(42); torch.manual_seed(42)
print(f"v5X: lf~U({LF_MIN},{LF_MAX}), K={K_FIXED}")
...
def gen_batch(rng, n):
    ...
    for _ in range(n):
        ...
        V = V_library[ti,lf_idx,:]; obs = np.sort(rng.choice(range(1,N_BUS),K_FIXED,replace=False))
...
for step in range(1, N_STEPS+1):
    ...
    if step % LOG_EVERY == 0:
        print(f"  {step:>8}  {loss.item():>8.4f}  {time.time()-t0:>7.1f}s")
...
for _ in range(1000):
    ...
    V=V_library[ti,lf_idx,:]; obs=np.sort(rng_te.choice(range(1,N_BUS),K_FIXED,replace=False))
...
print(f"\n{'='*60}")
print(f"v5X  lf~U({LF_MIN},{LF_MAX})  K={K_FIXED}")
print(f"  AIS top-1: {np.mean(acc_ais):.3f}   NRE top-1: {np.mean(acc_nre):.3f}   gap: {np.mean(acc_ais)-np.mean(acc_nre):+.3f}")
print(f"  KL: {np.mean(kl_vals):.4f}   CI-90: {np.mean(ci90):.3f}")
print(f"  AIS timing: {ais_ms:.1f}ms   NRE timing: {nre_ms:.3f}ms   Speedup: {ais_ms/nre_ms:.0f}x")
print(f"  [{'OK' if np.mean(acc_nre)>=0.78 else 'FAIL'}] NRE>=78%  [{'OK' if np.mean(acc_ais)>=0.80 else 'FAIL'}] AIS>=80%  [{'OK' if ais_ms>=900 else 'FAIL'}] AIS>=900ms  [{'OK' if nre_ms<2 else 'FAIL'}] NRE<2ms  [{'OK' if np.mean(kl_vals)<0.05 else 'FAIL'}] KL<0.05")
print(f"{'='*60}")
