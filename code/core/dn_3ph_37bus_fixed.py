...
OUT_PATH = os.path.join(SAVE_DIR, 'threephase_37bus_fixed_result.txt')
SEEDS = [42, 123, 456]
SIGMA = 0.005
...
class NRE3ph(nn.Module):
    def __init__(self, in_dim, n_topo):
        super().__init__()
        self.embed = nn.Sequential(nn.Linear(in_dim, 384), nn.LayerNorm(384), nn.GELU())
        self.res1 = ResBlock(384)
        self.res2 = ResBlock(384)
        self.head = nn.Sequential(nn.Linear(384, 192), nn.LayerNorm(192), nn.GELU(), nn.Linear(192, n_topo))
    def forward(self, x):
        h = self.embed(x)
        h = self.res1(h)
        h = self.res2(h)
...
INSTALLED = np.unique(np.rint(np.linspace(1, N_BUS - 1, K)).astype(int))
K = len(INSTALLED)
...
def main():
    ...
    lines = [f'Device={DEVICE}', f'Benchmark=37bus_fixed n_bus={N_BUS} K={K} raw_topos={len(raw_topos)} kept_topos={n_topos} failed_pf={failed}']
    rows = []
    for seed in SEEDS:
        ...
        with open(OUT_PATH, 'w', encoding='utf-8') as f:
            f.write('\n'.join(lines))
        torch.save({'model_state': model.state_dict(), 'seed': seed, 'name': '37bus_fixed', 'n_topos': n_topos}, os.path.join(SAVE_DIR, f'nre_3ph_37bus_fixed_seed{seed}.pt'))
    ...
    lines.append('- Synthetic unbalanced three-phase 37-bus fixed-deployment benchmark.')
    lines.append('- Stronger than the earlier random-mask pilot for assessing learned-posterior scalability under a stable sensor layout.')
    with open(OUT_PATH, 'w', encoding='utf-8') as f:
        f.write('\n'.join(lines))
    print(OUT_PATH)
