# -*- coding: utf-8 -*-
"""
IP-C 负荷感知版本：通信缺失鲁棒性实验
框架：K=20, lf~U(0.8,1.2)，与v5A一致
对比：鲁棒NRE（mask增广训练）vs 朴素NRE（v5A）vs AIS
测试：missing_rate = 0%, 5%, 10%, 20%, 30%
"""
...
print(f"IP-C load-aware: K={K_FIXED}, lf~U({LF_MIN},{LF_MAX}), missing~U(0,0.3)")
...
# ── 加载朴素NRE（v5A）────────────────────────────────────────────────────────
print("Loading naive NRE (v5A)...")
ckpt_naive = torch.load(NAIVE_PATH, map_location=DEVICE, weights_only=False)
naive_model = LoadAwareNRE(N_TOPOS, N_BUS).to(DEVICE)
naive_model.load_state_dict(ckpt_naive['model_state']); naive_model.eval()
print("Naive NRE loaded.")

# ── 训练鲁棒NRE（mask增广）───────────────────────────────────────────────────
def gen_batch_robust(rng, n):
    """训练时随机mask部分安装节点，模拟通信缺失"""
    xs, ys = [], []
    for _ in range(n):
...
def ais_posterior(reported, obs_v, lf_idx):
    if len(reported) == 0:
        return np.ones(N_TOPOS) / N_TOPOS
    V_topos = V_library[:, lf_idx, :]
    diff = (V_topos[:, reported] - obs_v) / SIGMA
    ll = -0.5 * np.sum(diff**2, axis=1); ll -= ll.max()
    w = np.exp(ll); return w / w.sum()

# ── 测试 ─────────────────────────────────────────────────────────────────────
robust_model.eval()
print(f"\nEvaluating at missing rates: {MISSING_RATES}")
print(f"{'Miss%':>6}  {'AIS':>7}  {'Robust':>8}  {'Naive':>7}  {'Delta_R':>9}  {'Delta_N':>9}  {'H_robust':>9}  {'H_naive':>8}")
print("-"*75)
...
for miss_rate in MISSING_RATES:
    ...
    h_rob, h_nai = [], []
    rng_te2 = np.random.RandomState(77)  # 固定seed确保每个miss_rate用同样的场景
    for _ in range(1000):
        ti     = rng_te2.randint(0, N_TOPOS)
        lf_idx = rng_te2.randint(0, N_LF)
        lf     = lf_grid[lf_idx]
        V      = V_library[ti, lf_idx, :]
        installed = np.sort(rng_te2.choice(range(1, N_BUS), K_FIXED, replace=False))
        if n_miss > 0:
            miss_idx = rng_te2.choice(len(installed), n_miss, replace=False)
            reported = np.delete(installed, miss_idx)
        else:
            reported = installed
        obs_v = V[reported] + rng_te2.normal(0, SIGMA, len(reported))
        # AIS
        p_ais = ais_posterior(reported, obs_v, lf_idx)
        acc_ais.append(int(np.argmax(p_ais) == ti))
        # Robust NRE
        p_rob = infer_nre(robust_model, reported, obs_v, lf, lf_idx)
        acc_rob.append(int(np.argmax(p_rob) == ti))
        h_rob.append(-np.sum(p_rob * np.log(p_rob + 1e-10)))
        # Naive NRE (v5A，用全部安装节点，不知道缺失)
        obs_v_full = V[installed] + rng_te2.normal(0, SIGMA, K_FIXED)
        p_nai = infer_nre(naive_model, installed, obs_v_full, lf, lf_idx)
        acc_nai.append(int(np.argmax(p_nai) == ti))
        h_nai.append(-np.sum(p_nai * np.log(p_nai + 1e-10)))

    a_ais = np.mean(acc_ais); a_rob = np.mean(acc_rob); a_nai = np.mean(acc_nai)
    results.append({'miss': miss_rate, 'ais': a_ais, 'robust': a_rob, 'naive': a_nai,
                    'h_rob': np.mean(h_rob), 'h_nai': np.mean(h_nai)})
    print(f"  {miss_rate*100:>4.0f}%  {a_ais:>7.3f}  {a_rob:>8.3f}  {a_nai:>7.3f}  "
          f"{a_rob-a_nai:>+9.3f}  {a_ais-a_nai:>+9.3f}  "
          f"{np.mean(h_rob):>9.3f}  {np.mean(h_nai):>8.3f}", flush=True)

print(f"\n{'='*60}")
print(f"IP-C Load-Aware Results Summary")
print(f"{'Miss%':>6}  {'Robust':>8}  {'Naive':>8}  {'Delta(R-N)':>12}")
print("-"*40)
for r in results:
    print(f"  {r['miss']*100:>4.0f}%  {r['robust']:>8.3f}  {r['naive']:>8.3f}  {r['robust']-r['naive']:>+12.3f}")
delta_10 = results[2]['robust'] - results[2]['naive']
delta_30 = results[4]['robust'] - results[4]['naive']
print(f"\n  Key: miss=10%: Robust-Naive={delta_10:+.3f}")
print(f"       miss=30%: Robust-Naive={delta_30:+.3f}")
gate = delta_10 > 0.10 and delta_30 > 0.10
print(f"\n  Gate (Delta>10pp at 10% and 30%): {'PASS' if gate else 'FAIL'}")
print(f"{'='*60}", flush=True)

torch.save({'model_state': robust_model.state_dict(), 'N_TOPOS': N_TOPOS,
