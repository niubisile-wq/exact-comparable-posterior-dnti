# -*- coding: utf-8 -*-
"""v5Z: K=20, 100k步, lf~U(0.7,1.3) — 冲过两条未达标准"""
...
print(f"v5Z: lf~U({LF_MIN},{LF_MAX}), K={K_FIXED}, steps={N_STEPS}")
...
print(f"\n{'='*60}", flush=True)
print(f"v5Z  lf~U({LF_MIN},{LF_MAX})  K={K_FIXED}  steps={N_STEPS}", flush=True)
...
print(f"  [{'OK' if nre_ok else 'FAIL'}] NRE>=AIS-5pp  [{'OK' if ais_ok else 'FAIL'}] AIS>=75%  [{'OK' if ais_t_ok else 'FAIL'}] AIS>=900ms  [{'OK' if nre_t_ok else 'FAIL'}] NRE<3ms  [{'OK' if kl_ok else 'FAIL'}] KL<0.05", flush=True)
