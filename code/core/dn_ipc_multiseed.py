# -*- coding: utf-8 -*-
"""IP-C 多seed验证：seed=42已有，补跑123/456，报均值±std"""
...
SEEDS     = [123, 456]   # seed=42已完成，复用结果
print(f"Device: {DEVICE}  IP-C multi-seed: {SEEDS}")
...
all_results = {
    42: [
        {'miss':0.00,'rob':0.678,'nai':0.670,'delta':+0.008},
        {'miss':0.05,'rob':0.667,'nai':0.561,'delta':+0.106},
        {'miss':0.10,'rob':0.654,'nai':0.401,'delta':+0.253},
        {'miss':0.20,'rob':0.626,'nai':0.214,'delta':+0.412},
        {'miss':0.30,'rob':0.615,'nai':0.101,'delta':+0.514},
    ]
}
print(f"\nSeed 42 (pre-computed): miss=10% delta=+0.253, miss=30% delta=+0.514")
...
for seed in SEEDS:
    ...
    torch.save({'model_state':rob_model.state_dict(),'seed':seed,
                'N_TOPOS':N_TOPOS,'K_FIXED':K_FIXED,'SIGMA':SIGMA},
               f"{SAVE_DIR}\\nre_ipc_seed{seed}.pt")

# ── 汇总 ─────────────────────────────────────────────────────────────────────
print(f"\n{'='*60}")
print(f"IP-C MULTI-SEED SUMMARY (3 seeds: 42,123,456)")
print(f"{'Miss%':>6}  {'Rob_mean':>9}  {'Rob_std':>8}  {'Nai_mean':>9}  {'Delta_mean':>11}  {'Delta_min':>10}")
print("-"*60)
for i,miss_rate in enumerate(MISSING_RATES):
    robs=[all_results[s][i]['rob'] for s in [42]+SEEDS]
    nais=[all_results[s][i]['nai'] for s in [42]+SEEDS]
    deltas=[all_results[s][i]['delta'] for s in [42]+SEEDS]
...
# 闸门：所有seed在10%和30%时delta>10pp
gate_10=all(all_results[s][2]['delta']>0.10 for s in [42]+SEEDS)
gate_30=all(all_results[s][4]['delta']>0.10 for s in [42]+SEEDS)
...
print(f"  {'*** IP-C MULTI-SEED STABLE ***' if gate_10 and gate_30 else '--- Not stable ---'}", flush=True)
print(f"{'='*60}")
