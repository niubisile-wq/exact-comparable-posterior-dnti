# -*- coding: utf-8 -*-
"""
119-bus IP1 sensor-policy v2 training.

Uses the diversity-constrained searched fixed-25 sensor policy selected by
exact-reference validation in large_system_sensor_policy_search_20260702.

This run tests whether the already-improved sensor policy can also support a
specialized learned posterior with higher direct top-1 accuracy.
"""
...
SENSORS = np.array([5, 8, 11, 19, 23, 27, 34, 36, 44, 49, 51, 57, 62, 67, 74, 76, 84, 88, 93, 95, 102, 106, 109, 111, 118], dtype=np.int64)
SIGMA = 0.009
BATCH = 1024
N_TRAIN = 120000
N_VAL = 4000
...
OUT_NAME = "119bus_sensor_policy_v2_train_20260702.txt"
...
def train_one(seed, lib, xs_train, y_true_train, y_map_train, xs_val, y_true_val, y_map_val):
    ...
    rng = np.random.RandomState(119000 + seed)
    best_score = -1e9
    ...
    ckpt_name = f"nre_119bus_ip1_sensor_v2_seed{seed}_20260702.pt"
    torch.save(
        {
...
def main():
    ...
    print(f"119-bus sensor-policy v2 training: train={N_TRAIN} val={N_VAL} final={N_FINAL}")
    xs_train, y_true_train, y_map_train = make_dataset(lib, N_TRAIN, 119801)
    xs_val, y_true_val, y_map_val, ll_val = make_dataset(lib, N_VAL, 119802, return_ll=True)
    xs_final, y_true_final, y_map_final, ll_final = make_dataset(lib, N_FINAL, 119803, return_ll=True)
    ...
    for seed in SEEDS:
        ...
        model, ckpt_name, best_epoch, best_val = train_one(seed, lib, xs_train, y_true_train, y_map_train, xs_val, y_true_val, y_map_val)
    ...
    old_rr = rerank_metrics(p_old_final, y_true_final, y_map_final, ll_final, 20)
    new_rr = rerank_metrics(p_new_final, y_true_final, y_map_final, ll_final, 20)
    mix_rr = rerank_metrics(p_mix_final, y_true_final, y_map_final, ll_final, 20)

    lines = []
    lines.append("119-bus IP1 searched sensor-policy v2 training")
    lines.append("date=2026-07-02")
    lines.append("status=isolated_sensor_policy_v2_does_not_overwrite_original_checkpoints")
    lines.append(f"device={DEVICE}")
    lines.append("sensors=" + " ".join(map(str, SENSORS.tolist())))
    lines.append(f"train_samples={N_TRAIN}")
    lines.append(f"validation_samples={N_VAL}")
    lines.append(f"final_test_samples={N_FINAL}")
    lines.append("loss=CE(true_topology)+0.20*CE(exact_MAP)")
    ...
    lines.append(f"old_probability_ensemble_truth_in_top20={old_rr['truth_in_top20']:.4f}")
    lines.append(f"old_probability_ensemble_map_in_top20={old_rr['map_in_top20']:.4f}")
    lines.append(f"old_probability_ensemble_rerank20_true_acc={old_rr['rerank20_true_acc']:.4f}")
    ...
    lines.append(f"v2_probability_ensemble_truth_in_top20={new_rr['truth_in_top20']:.4f}")
    lines.append(f"v2_probability_ensemble_map_in_top20={new_rr['map_in_top20']:.4f}")
    lines.append(f"v2_probability_ensemble_rerank20_true_acc={new_rr['rerank20_true_acc']:.4f}")
    ...
    lines.append(f"validation_selected_mix_truth_in_top20={mix_rr['truth_in_top20']:.4f}")
    lines.append(f"validation_selected_mix_map_in_top20={mix_rr['map_in_top20']:.4f}")
    lines.append(f"validation_selected_mix_rerank20_true_acc={mix_rr['rerank20_true_acc']:.4f}")
    lines.append(f"delta_v2_vs_old_final={new_final['true_acc'] - old_final['true_acc']:.4f}")
