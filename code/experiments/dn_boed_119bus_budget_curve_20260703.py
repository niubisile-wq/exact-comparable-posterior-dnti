# -*- coding: utf-8 -*-
"""
119-bus adaptive BOED multi-budget decision-value curve.

This extends the single-point K=4/K=7 bootstrap audit into a budget curve
K=3/4/5/6/7/8/10 with Random, AdaptiveMVG, AdaptiveFisher, and AdaptiveBOED.
It reports per-budget bootstrap CIs plus an AUC-style budget-curve summary.
...
OUT_NAME = "boed_119bus_budget_curve_20260703.txt"

SIGMA = 0.009
N_LF_C = 11
K_REPORT = [3, 4, 5, 6, 7, 8, 10]
K_MAX = 10
N_TEST = 90
N_MC = 32
BOOT = 1200
...
def auc_over_budget(vals_by_k):
    vals = np.array([vals_by_k[k] for k in K_REPORT], dtype=np.float64)
...
def main():
    ...
    for ci, (true_ti, _lf_idx, full_v) in enumerate(test_cases):
        ...
        per_case_acc = {m: {} for m in methods}
        per_case_ent = {m: {} for m in methods}
        for name, order in orders.items():
            for k in K_REPORT:
                ok, h = evaluate_order(v_flat_c, order, full_v, true_ti, n_topos, N_LF_C, k)
                acc[name][k].append(ok)
                ent[name][k].append(h)
                per_case_acc[name][k] = ok
                per_case_ent[name][k] = h
        for name in methods:
            auc_acc[name].append(auc_over_budget(per_case_acc[name]))
            auc_ent[name].append(auc_over_budget(per_case_ent[name]))

    lines = []
    lines.append("119-bus Adaptive BOED multi-budget decision-value curve")
    lines.append("date=2026-07-03")
    lines.append("role=budget-curve BOED/MVG/Fisher/Random comparison with bootstrap CIs")
    ...
    lines.append("per_budget_metrics")
    ...
    lines.append("budget_curve_auc")
    lines.append("metric,method_or_delta,mean,ci95_low,ci95_high")
    for name in methods:
        mean, lo, hi = boot_mean(auc_acc[name])
        lines.append(f"accuracy_auc,{name},{mean:.6f},{lo:.6f},{hi:.6f}")
    for name in methods:
        mean, lo, hi = boot_mean(auc_ent[name])
        lines.append(f"entropy_auc,{name},{mean:.6f},{lo:.6f},{hi:.6f}")
    for other in ["Random", "AdaptiveMVG", "AdaptiveFisher"]:
        diff_acc = np.asarray(auc_acc["AdaptiveBOED"], dtype=float) - np.asarray(auc_acc[other], dtype=float)
        mean, lo, hi = boot_mean(diff_acc)
        lines.append(f"accuracy_auc_delta_BOED_minus_{other},AdaptiveBOED-{other},{mean:.6f},{lo:.6f},{hi:.6f}")
        diff_ent = np.asarray(auc_ent[other], dtype=float) - np.asarray(auc_ent["AdaptiveBOED"], dtype=float)
        mean, lo, hi = boot_mean(diff_ent)
        lines.append(f"entropy_auc_reduction_BOED_vs_{other},{other}-AdaptiveBOED,{mean:.6f},{lo:.6f},{hi:.6f}")
