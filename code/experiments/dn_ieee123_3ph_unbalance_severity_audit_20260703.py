# -*- coding: utf-8 -*-
"""
IEEE123 three-phase unbalance-severity audit.

Amplifies per-bus phase asymmetry at the observation level and measures whether
the posterior pipeline remains usable as severity increases, with and without
dropout.
"""
...
OUT_NAME = "ieee123_3ph_unbalance_severity_audit_20260703.txt"
DEVICE = m123.DEVICE
K_BASE = 75
SEEDS = [42, 123, 456]
BATCH = 256
...
SCENARIOS = [
    ("nominal", 0.00, 1.0, 1.0),
    ("unbalance15", 0.00, 1.0, 1.5),
    ("unbalance20", 0.00, 1.0, 2.0),
    ("unbalance30", 0.00, 1.0, 3.0),
    ("drop20_unbalance15", 0.20, 1.0, 1.5),
    ("drop20_unbalance20", 0.20, 1.0, 2.0),
    ("drop20_unbalance30", 0.20, 1.0, 3.0),
]
...
def amplify_unbalance(obs_v3, mult):
    if mult == 1.0:
        return obs_v3
    center = np.mean(obs_v3, axis=1, keepdims=True)
    return center + mult * (obs_v3 - center)


def mean_bus_unbalance(obs_v3):
    phase_max = np.max(obs_v3, axis=1)
    phase_min = np.min(obs_v3, axis=1)
    phase_mean = np.mean(obs_v3, axis=1)
    return float(np.mean((phase_max - phase_min) / np.clip(phase_mean, 1e-9, None)))


def make_dataset(lib, n, seed, drop_rate, noise_mult, unbalance_mult):
    ...
    for i in range(n):
        ...
        obs = amplify_unbalance(obs, unbalance_mult)
        ...
        imbalance[i] = mean_bus_unbalance(obs)
    return xs, y, q, retained, imbalance
...
def metrics(p, y, q):
    ...
    out = {
        ...
        "posterior_entropy": mean_entropy(p),
        "exact_entropy": mean_entropy(q),
    }
    arrays = {"direct_correct": direct == y}
    for k in [5, 10, 20]:
        cand = top[:, :k]
...
def boot_ci_diff(a, b, boot=1000):
    ...
    rng = np.random.RandomState(1308)
...
def main():
    ...
    lines = []
    lines.append("IEEE123 three-phase unbalance-severity audit")
    lines.append("date=2026-07-03")
    lines.append("role=observation-level amplification of per-bus phase asymmetry for controlled unbalanced IEEE123 posterior inference")
    lines.append("not_claimed=utility field data; larger feeder scale")
    ...
    lines.append("scenario,drop_rate,unbalance_mult,retained_K,mean_obs_unbalance,model,exact_top1,direct_top1,exact_agree,kl,posterior_entropy,exact_entropy,truth_top5,truth_top10,truth_top20,rerank5,rerank10,rerank20,direct_gain_ci_vs_fixed,top20_gain_ci_vs_fixed,rerank20_gain_ci_vs_fixed")

    for si, (name, drop_rate, noise_mult, unbalance_mult) in enumerate(SCENARIOS):
        xs, y, q, retained, imbalance = make_dataset(lib, N_TEST, 991500 + si * 79, drop_rate, noise_mult, unbalance_mult)
        ...
        for label, met, arr in results:
            ...
            lines.append(
                f"{name},{drop_rate:.2f},{unbalance_mult:.1f},{int(np.mean(retained))},{float(np.mean(imbalance)):.4f},{label},"
                f"{met['exact_top1']:.4f},{met['direct_top1']:.4f},{met['exact_agree']:.4f},{met['kl_ref_nre']:.4f},"
                f"{met['posterior_entropy']:.4f},{met['exact_entropy']:.4f},{met['truth_top5']:.4f},{met['truth_top10']:.4f},"
                f"{met['truth_top20']:.4f},{met['rerank5']:.4f},{met['rerank10']:.4f},{met['rerank20']:.4f},"
                f"{dg},{tg},{rg}"
            )
