# -*- coding: utf-8 -*-
"""
IEEE123 three-phase phase-topology ambiguity audit.

Injects global and partial phase-permutation errors at the observation level and
measures how posterior inference degrades, how entropy rises, and whether
candidate/rerank remains near-exact.
"""
...
OUT_NAME = "ieee123_3ph_phase_ambiguity_audit_20260703.txt"
DEVICE = m123.DEVICE
K_BASE = 75
SEEDS = [42, 123, 456]
BATCH = 256
...
SCENARIOS = [
    ("nominal", 0.00, 1.0, "none"),
    ("global_swap_ab", 0.00, 1.0, "global_swap_ab"),
    ("global_cycle_abc", 0.00, 1.0, "global_cycle_abc"),
    ("local20_swap_ab", 0.00, 1.0, "local20_swap_ab"),
    ("local40_swap_ab", 0.00, 1.0, "local40_swap_ab"),
    ("drop20_local20_swap_ab", 0.20, 1.0, "local20_swap_ab"),
]
...
def apply_phase_mode(obs, mode, rng):
    out = obs.copy()
    n = len(out)
    if mode == "none":
        return out
    if mode == "global_swap_ab":
        out = out[:, [1, 0, 2]]
        return out
    if mode == "global_cycle_abc":
        out = out[:, [1, 2, 0]]
        return out
    if mode == "local20_swap_ab":
        k = max(1, int(round(0.20 * n)))
        idx = rng.choice(n, size=k, replace=False)
        out[idx] = out[idx][:, [1, 0, 2]]
        return out
    if mode == "local40_swap_ab":
        k = max(1, int(round(0.40 * n)))
        idx = rng.choice(n, size=k, replace=False)
        out[idx] = out[idx][:, [1, 0, 2]]
        return out
...
def make_dataset(lib, n, seed, drop_rate, noise_mult, phase_mode):
    ...
    for i in range(n):
        ...
        obs = apply_phase_mode(obs, phase_mode, rng)
...
def boot_ci_diff(a, b, boot=1000):
    ...
    rng = np.random.RandomState(1307)
...
def main():
    ...
    lines = []
    lines.append("IEEE123 three-phase phase-topology ambiguity audit")
    lines.append("date=2026-07-03")
    lines.append("role=observation-level phase permutation stress for controlled unbalanced IEEE123 posterior inference")
    lines.append("not_claimed=joint phase identification; utility field validation")
    ...
    lines.append("scenario,drop_rate,phase_mode,retained_K,model,exact_top1,direct_top1,exact_agree,kl,posterior_entropy,exact_entropy,truth_top5,truth_top10,truth_top20,rerank5,rerank10,rerank20,direct_gain_ci_vs_fixed,top20_gain_ci_vs_fixed,rerank20_gain_ci_vs_fixed")

    for si, (name, drop_rate, noise_mult, phase_mode) in enumerate(SCENARIOS):
        xs, y, q, retained = make_dataset(lib, N_TEST, 990000 + si * 83, drop_rate, noise_mult, phase_mode)
        ...
        for label, met, arr in results:
            ...
            lines.append(
                f"{name},{drop_rate:.2f},{phase_mode},{int(np.mean(retained))},{label},"
                f"{met['exact_top1']:.4f},{met['direct_top1']:.4f},{met['exact_agree']:.4f},{met['kl_ref_nre']:.4f},"
                f"{met['posterior_entropy']:.4f},{met['exact_entropy']:.4f},{met['truth_top5']:.4f},"
                f"{met['truth_top10']:.4f},{met['truth_top20']:.4f},{met['rerank5']:.4f},{met['rerank10']:.4f},{met['rerank20']:.4f},"
                f"{dg},{tg},{rg}"
            )
